/**
 * ElevenLabs Speech-to-Text route.
 *
 * Request:
 *   POST /api/elevenlabs/transcribe?languageCode=<optional>&modelId=<optional>
 *   Content-Type: audio/webm | audio/mp4 | audio/wav | …
 *   Body: raw audio bytes (no multipart wrapper)
 *
 * Response:
 *   200 { text, languageCode?, languageProbability?, audioDurationSecs?, raw }
 */
...
export default async function handler(req: AnyReq, res: AnyRes): Promise<void> {
  ...
  const modelId = getQueryParam(req, "modelId")?.trim() || undefined;
